...
**第 0 步：固化验证入口。** 把 `PROJECT-FACTS.md` §构建/验证命令固化成可执行的 `project/verify.sh`（macOS/Linux）或 `project/verify.ps1`（Windows）——无占位符；含 JDK/工具链选择等本机事实；只做只读构建/测试，不改代码不发布。写完跑一次 `bash ai-infra/tools/aci.sh verify`（Windows: `ai-infra\tools\aci.cmd verify`）确认真的通过——这是 agent 以后收尾的一键门禁。
