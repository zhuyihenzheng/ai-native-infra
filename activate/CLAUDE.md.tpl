...
- **ACI 门禁**（领域命令，观察仓库仍用你的原生 Grep/Glob/Read）：开工先 `bash {{INFRA_DIR}}/tools/aci.sh state`；改 spec/DEF 前用 `trace <ID>` 算爆炸半径；核查证据引用用 `evidence <path:line>`；收尾必跑 `verify`（本项目对齐的 build/test 一键验证）+ `validate`。
...
- **验证纪律**：改动后必须跑 `bash {{INFRA_DIR}}/tools/aci.sh verify`（即核心契约命令段固化成的 `project/verify.sh`）；涉及画面/接口的改动，按对齐规则做端到端验证，不要只靠编译通过。
